...
interface PaymentProps{
    ...
    inStore: boolean,
    setBuyerInfo: Dispatch<SetStateAction<{
        name: string;
        city: string;
        address: string;
    }>>
...
export default function PaymentDetails({
    delivery,setDelivery,setInStore,inStore,setBuyerInfo
}:PaymentProps) {
  ...
  return (
    ...
    <div className="w-full h-fit border border-black-2 p-1 mt-2"><CartTable actions={false} payments={true}/></div>
    ...
    <p className="font-bold text-sm mt-2" role="alert">Ciudad:</p>
          <select {...register("city", { required: true, maxLength: 20 })}  defaultValue={'Bogotá D.C'} aria-invalid={errors.city ? "true" : "false"} >
      ...
      <input {...register("address", { required: true, maxLength: 20 })}  aria-invalid={errors.address ? "true" : "false"} placeholder={'Escribe la direccion'}/>
      {errors.address?.type === "required" && (
        <p className="text-red-700 font-bold text-sm" role="alert">Direccion  es mandatorio!</p>
      )}
     <div className="flex justify-center mt-4"><Button type="normal" name="Confirmar" size="50%" position="center"/></div>
        </form>
        </>
        }
    {inStore && 
